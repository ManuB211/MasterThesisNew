...
public class FragmentUtil {
    ...
    public static <E extends Edge<N>, N extends INode> IRPSTNode<Edge<IChoreographyNode>, IChoreographyNode> getCorrespondingGlobalFragment(IRPSTNode<E,N> f, ChoreographyModel g) {
     ...
     */
    // TODO: this is exactly the same logic as getCorrespondingGlobalFragment but with different types set....refactor
    public static <E extends Edge<N>, N extends INode> IRPSTNode<E,N> getCorrespondingLocalFragment(IRPSTNode<Edge<IChoreographyNode>, IChoreographyNode> f, RpstModel<E,N> localModel) {
    ...
    public static <E extends Edge<N>, N extends INode> Set<Message> collectMessages(IRPSTNode<E,N> f) {
    	...
    	// TODO: i'm pretty sure there are null messages here too......should we delete them?
    	messages.remove(null);
    	return messages;
    }
    
    /**
     * returns a set of activities, which are associated with the role inside a fragment
     */
    ...
    public static<E extends Edge<N>, N extends INode> IRpstModel<E, N> projectRole(IRPSTNode<E,N> f, Role r, IDirectedGraph<E,N> g, boolean doGraphReduce) {
        N start = f.getEntry();
        N end = f.getExit();

        //System.out.println("FragmentUtil.projectRole on Fragment:" + f + " role:" + r + " graph:" + g + " doGraphReduce:" + doGraphReduce);
        ...
        for (E edge : f.getFragment()) {
            ...
            for (Object x : new LinkedList(Arrays.asList(edge.getSource(), edge.getTarget()))) {
                ...
                if ((n instanceof Event) || (n instanceof Gateway)) {
                    ...
                    sourceAdd=true;
                } else if (n instanceof Interaction && (((Interaction) n).getSender().equals(r) || ((Interaction) n).getReceiver().equals(r))) {
                    sourceAdd=true;
                }
                ...
                if (sourceAdd) {
                    ...
                    for (N successor: FragmentUtil.getNextRoleActivities(n, r, g, end)) {
                      //  System.out.println("(FragmentUtil.projectRole) Adding edge to graph: " + n + " <-> " + successor);
       ...
       // System.out.println("(FragmentUtil.projectRole) building RpstModel using graph: " + graph);

        // sometimes the graph is empty; this occurs when the projection returns nothing.
    ...
    public static<E extends Edge<N>, N extends INode> List<N> getNextRoleActivities(N node, Role r, IDirectedGraph g, N endNode) {
        List<N> L = new LinkedList<N>();

        if (endNode == node) return L;
    ...
    public static <E extends Edge<N>, N extends INode> boolean matchingPaths(MultiDirectedGraph<E,N> g1, N n1, MultiDirectedGraph<E,N> g2, N n2) {
...
/*
        ...
        if (children1.size() != children2.size()) {
            //System.out.println("### (short circuit) Size of childrens don't match: " + children1.size() + " vs " + children2.size());
            return false;
        }
       ...
       /* System.out.println("XORgateways1: " + XORgateways1 + " " + XORgateways1.size());
        System.out.println("XORgateways2: " + XORgateways2 + " " + XORgateways2.size());
        System.out.println("ANDgateways1: " + ANDgateways1 + " " + ANDgateways1.size());
        System.out.println("ANDgateways2: " + ANDgateways2 + " " + ANDgateways2.size());
        System.out.println("activities1: " + activities1 + " " + activities1.size());
        System.out.println("activities2: " + activities2 + " " + activities2.size());
*/
        // if the node type counts are different, we can break off
        if (XORgateways1.size() != XORgateways2.size() &&
            ANDgateways1.size() != ANDgateways2.size() &&
            activities1.size() != activities2.size()) {
            //System.out.println("### (short circuit) Size of children types don't match.");
            return false;
        }
        ...
        for (N x1 : activities1) { activityIds1.add(x1.getId()); }
        for (N x2 : activities2) { activityIds2.add(x2.getId()); }
    ...
    /*public static <E extends Edge<N>,N extends INode> RpstModel<E,N> generateModelFromFragment(IRPSTNode<E,N> f){
    ...
    */
    ...
    /**
    	This function generates an rpst model from a fragment. It clones the elements of a fragment, generates the graph and encapsulates
    	it with start and end events. This function is used to generate correct models to test insert or replace change operations.
	*/
  	public static <E extends Edge<N>,N extends INode> RpstModel<E,N> generateCloneModelFromFragment(IRPSTNode<E,N> f, RpstModel<E,N> m){
	...
	public static <E extends Edge<N>,N extends INode> RpstModel<E,N> generateCloneModelFromNode(N node){
  	...
  	/**
  	 * Returns the set of models corresponding to all the fragments of a given model (by adding end and start events)
  	 * @param m
  	 * @return
  	 */
  	public static <E extends Edge<N>,N extends INode> Set<RpstModel<E,N>> getAllCloneFragmentModels(RpstModel<E,N> m){
    ...
    /**
     * This Function computes the complement of a given fragment in the current model
