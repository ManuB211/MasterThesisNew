...
public class Bpmn2ChoreographyModel {
	...
	protected void initGraph() {
		...
		// finding the roles + normalization: participants having the same name are
		// added as a same role with a same ID
		// Indeed, for two different interactions I1 and I2 that use a same role R,
		// Signavio generates a new ID for R in
		...
		for (Object role : choreography.getChildren("participant", BPMN2NS)) {
			...
			if (exists == false)
				roles.add(
						new Role(((Element) role).getAttributeValue("name"), ((Element) role).getAttributeValue("id")));
			exists = false;
		}
		for (Role r : roles)
			ID2RoleMap.put(r.id, r);

		// we parse the participants and build the Roles, then the interactions,
		// gateways and events.
		for (Object obj : choreography.getChildren()) {
			if (obj instanceof Element) {
				...
				if (elem.getName().equals("startEvent") || elem.getName().equals("endEvent")) {
					...
					/*
					 * TODO Multiple Messages Approach List<Element> chorMsgRefs; Set<Message>
					 ...
					 * int msgCount = 0;
					 * 
					 * for (Element msg : chorMsgRefs) { msgCount++; String msgRef = msg.getValue();
					 * Triple t = messageFlowMap.get(msgRef); Message message=null; Role sourceRole=
					 * null; Role targetRole = null;
					 * 
					 * for(Message m : messagelist) if(m.getId().equals(t.messageRef)) message = m;
					 * for(Role role : roles){ if(role.id.equals(ID2RoleMap.get(t.sourceRef).id))
					 * sourceRole= role; if(role.id.equals(ID2RoleMap.get(t.targetRef).id))
					 * targetRole = role; }
					 * 
					 * }
					 * 
...
//System.out.println(sourceRole.id+"=="+sourceRole.name+"-->"+targetRole.name+"=="+targetRole.id);
					...
					logger.warn("Unprocessed Element: " + elem.getName() + ", id: " + elem.getAttributeValue("id"));
			}
		}
