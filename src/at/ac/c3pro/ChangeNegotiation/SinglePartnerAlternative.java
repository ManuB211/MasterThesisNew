...
public class SinglePartnerAlternative {
	...
	public double utility(double privateweight, double publicweight, double commonweight) {
		utility = privateweight * this.target.privatecost.cost + publicweight * this.target.publiccost.cost
				+ commonweight * this.commoncost.cost;
		return utility;
	}

	public String toString() {
		return "source= (" + source.partner + ", PuCost " + source.toString() + ")    target = ("
				+ target.publiccost.partner + "," + target.toString() + ")";
	}
