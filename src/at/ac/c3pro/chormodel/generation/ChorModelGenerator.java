...
public class ChorModelGenerator {
	...
	public ChorModelGenerator(int participantCount, int interactionCount, int xorSplitCount, int andSpiltCount, int loopCount, int maxBranching) {
		super();
	...
	public MultiDirectedGraph<Edge<IChoreographyNode>, IChoreographyNode> build() {
		...
		IOUtils.toFile(sdf.format(timestamp) + "_enriched_graph" + ".dot", enrichedGraph.toDOT());
		insertLoops(loops);
	...
	private void currentGraphToDot(int loopNo) {
		Timestamp timestamp = new Timestamp(System.currentTimeMillis());
		IOUtils.toFile(sdf.format(timestamp) + "_autogen_" + loopNo + ".dot", buildGraph.toDOT());
	}
	...
	private void printCurrentInteractionState() {
		...
		System.out.println("ResTotal: " + determineResInteractions() + " ResGateways: " + getResInteractionsGateways() + " ResBuild: " + determineResInteractionsBuild());
