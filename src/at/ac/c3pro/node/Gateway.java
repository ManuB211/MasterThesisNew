...
public abstract class Gateway extends Node implements IGateway{
	...
	public boolean isJoin(RpstModel  model) {		
		if (model != null && model.getdigraph().getDirectPredecessors(this).size() > 1 && model.getdigraph().getDirectSuccessors(this).size() == 1){
			return true;
		}
	...
	public <E extends Edge<N>, N extends INode> boolean isJoin(MultiDirectedGraph<E,N> digraph) {		
		if (digraph != null && digraph.getDirectPredecessors((N)this).size() > 1 && digraph.getDirectSuccessors((N)this).size() == 1){
			return true;
		}
	...
	/*public Gateway clone(){
		if(this instanceof XorGateway)
			return new XorGateway(this.getName());
		if(this instanceof AndGateway)
