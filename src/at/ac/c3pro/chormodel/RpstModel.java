...
public  class RpstModel<E extends Edge<N>, N extends INode> extends RPST<E,N> implements IRpstModel<E, N> {
    ...
    // used in getFragmentWithSource, getFragmentWithTarget, getFragmentWithSourceOrTarget
    ...
    /**
     * common base function for getFragmentWith*, filters out the fragment with the given node using the boolean function supplied by the callable
     */
    private IRPSTNode<E,N> getFragmentWith(EdgeFilter<E> f) {
        ...
        if (acc.size() == 1) return acc.get(0);
        return this.getsmallestFragment(acc);
    }

    /** Retrieve the single fragment that starts from the node. Comparison is based on the node's unique id.
     */
    ...
    /**
     * return the common smallest fragment that holds the node in either source or target.
     */
    public IRPSTNode<E,N> getFragmentWithSourceOrTarget(final N searchedNode) {
        return this.getFragmentWith(new EdgeFilter<E>() {
                public boolean filter(E edge) {
                    return edge.getSource().getName().equals(searchedNode.getName()) || edge.getTarget().getName().equals(searchedNode.getName());
                }
            });
    }
    ...
    public IRPSTNode<E,N> getFragmentBoundedBy(final N sourceNode, final N targetNode) {
        return this.getFragmentWith(new EdgeFilter<E>() {
                public boolean filter(E edge) {
                    return edge.getSource().getName() == sourceNode.getName() || edge.getTarget().getName() == targetNode.getName();
                }
            });
    }

    /**
     * return the trivial edge (fragment) which has sourceNode in getSource() and targetNode in getTarget()
     */
    ...
    public IDirectedGraph<E,N> innerDelete(Set<IRPSTNode<E,N>> Sequencefragment, IDirectedGraph<E,N> graph) {
   		//we first check if the set contains anly trivial nodes and it corresponds to a connected sequence of nodes 
		...
		//Checking if the edges are in sequence and finding the entry and the exit of this sequence
		...
		// At this level it is sure that it is a correct sequence with Fentry and Fexit as boundaries, so we proceed to the delete

        // TODO: extract this sequence checking as a method to be reused wherever we need
		...
		//IDirectedGraph<E, N> newDiGraph = removefragment(Fragment,Fentry, Fexit, graph);
		//return new RpstModel<E,N>((MultiDirectedGraph<E, N>) newDiGraph);
        //return newDiGraph;
	 ...
	 */
	private IDirectedGraph<E,N> removefragment(Set<E> Fragment, N predecessor, N successor, IDirectedGraph<E,N> graph) {
    ...
    /**
     * Reduce the graph of the model by identifying fragments where reduction is possible.
     */
    public RpstModel<E,N> reduceGraph() {
        RpstModel<E,N> model = this;           
        // recursively reduce the graph until there are no more graph reductions possible
    ...
    public List<IRPSTNode<E,N>> getFragmentsBottomUp(List<IRPSTNode<E,N>> options, List<IRPSTNode<E,N>> accum) {
        ...
        // when there are no more options, return the reversed accumulator for the correctly ordered list of fragments
    ...
    public boolean reduceGraph(IRPSTNode<E,N> fragment) {
        ...
        if ( entry instanceof AndGateway && exit instanceof AndGateway ) {
            ...
            if (directConnection == 2) {
                // AND BOTH case
                //System.out.println("\tAND BOTH case -> reduction possible (remove this bond) and reconnect the relevant nodes");
            ...
            if ( source.isSplit(this) && target.isJoin(this) ) {
                if (directConnection == 2) {
                    //System.out.println("\tINSIDE AND BOTH case -> reduction possible (remove this bond) and reconnect the relevant nodes");
                } else if (directConnection == 1) {
            ...
            // XOR BOTH?
            if (directConnection == 2) {
                //System.out.println("\tXOR BOTH case -> reduction possible (remove this bond) and reconnect the relevant nodes");
	...
	/**
	 * returns the list of all rpst nodes of the sub-rpst-tree with the root element: rpstNode 
	 * @param <E>
	 * @param <N>
	 */
	...
	/**
	 * @return  This function returns a map between the edges of the original graph and and the trivial nodes of the corresponding
	Rpst. A trivial node in the rpst represents an edge.
	 */
	...
	/* this maps Edge - RpstNode (trivial nodes)
	public Map<E,IRPSTNode<E,N>> getOriginal2RpstMap(){
		Map<E,IRPSTNode<E,N>> M = new HashMap<E,IRPSTNode<E,N>>();
		
		for(IRPSTNode<E,N> e : this.getRPSTNodes(TCType.TRIVIAL))
			 for(E e1 : this.diGraph.getEdges())
				if(e.getEntry() == e1.getSource() && e.getExit()==e1.getTarget())
					M.put(e1,e);		
		return M;
	}*/
	...
	//returns the set of loops (rppstnode root of the loop) that contains the given node
	...
	public double ExecutionProbability(IRPSTNode<E,N> fragment, Map<Pair<N,N>, Double> branchingprobabilityMap){
		...
		/*for(IRPSTNode<E,N> loop : getLoopsOfRpstNode(fragment)){
			probability = probability * branchingprobabilityMap.get(loop.getExit());//to correct
		}
		*/
	...
	//TODO
	//returns the number of execution of a given fragment : average of execution of its nodes
    ...
    // TODO: possible refactor with getPreSet: difference getDirectPredecessors & getPreSet calls
    ...
    // TODO: refactor getPostSet, getPreSet, getTransitivePostSet, getTransitivePreSet to use one common base function
    ...
    // TODO: refactor common logic from getTransitivePreSetF (different: call to getTransitivePostSet)
    ...
    /**
     * @return  this function projects a given Model on a role. It returns a structured model where the nodes are all reltated to this role.
     */
    public IRpstModel<E,N> projectionRole(Role role, boolean doGraphReduce){
        ...
        //System.out.println("-- (projectionRole) this.diGraph.getVertices():" + this.diGraph.getVertices());
           ...
           // System.out.println("this type:" + this.getClass());
            if((n instanceof Event) || (n instanceof Gateway)) {
              //  System.out.println("-- true: gateway or event");
                ...
                var=true;
            } else if (( n instanceof Interaction ) && (((Interaction) n).getSender().name.equals(role.name) || ((Interaction) n).getReceiver().name.equals(role.name))){            
    ...
    /**
     * @return  this function returns the list of next nodes of type gateway, Event, InteractionActivity or Interactions having the Role role of a given node
     */
    ...
    /**
    * Insert a fragment between the nodes specified in `position`. The inserted fragment should be bounded by a start and end events
    */
	...
	/**
	 * Replace an existing fragment by a new fragment. The inserted fragment should be bounded by a start and end events
	 */
	
	public IDirectedGraph<E,N> innerReplace(IRPSTNode<E,N> oldfragment, IRPSTNode<E,N> newfragment, IDirectedGraph<E,N> graph){

        N oldFsourceNode = oldfragment.getEntry();
        N oldFtargetNode = oldfragment.getExit();

        //Deleting the old fragment (we can not use the delete function since it includes graph reduction that we dont need here)
	...
	public boolean isActivity(N node) {
		return false; // should be overriden by subclasses: PrivateModel, PublicModel and ChoreographyModel
	}
