...
public class PropagationThread implements Runnable {
    ...
    private final Map<IRole, Map<ChgOpType, Set<IChangeOperation>>> operations;
    ...
    public PropagationThread(String choreoPath, List<String> roleNames) throws Exception {
        Bpmn2ChoreographyModel BookTripChoreoModel = new Bpmn2ChoreographyModel(choreoPath,
    ...
    public void run() {
        ...
        for (IChangeOperation op : all_ops) {
            ...
            // TODO
            Map<Role, List<IChangeOperation>> partner2propagatedOperationsMap = ChangePropagationUtil.propagate(op,
                    null, null, chgOpId2Stats, centrality, PropagationGraphMetrics);
            this.ChangePropagationLogMap.put(op, partner2propagatedOperationsMap);
            long after = System.currentTimeMillis();
            long diff = after - before;
            executionTimeMap.put(op.getId(), diff);
            System.out.println("====== " + Thread.currentThread().getName() + " (" + nb + "/" + nb_total + ") " + diff
                    + "ms " + op.getType());
