...
public class Bpmn2Collaboration {
	...
	// log4j ---
		...
		protected void initGraph() {
			...
			Set<IPublicNode> pumNodes = new HashSet<IPublicNode>(); //contains all created nodes of all public models: needed for building the map pu2puNode
					 ...
					 if( nodeElem.getName().equals("startEvent") || nodeElem.getName().equals("endEvent")){
						            pumNodes.add(new Event(nodeName,nodeId));
                                        ...
                                        System.out.println("targetRole: " + targetRole);
								 		...
								 		} else {if (logger.isInfoEnabled()) logger.info("mismatching send task"+ nodeElem);}
                                        ...
                                        System.out.println("sourceRole: " + sourceRole);
							 			...
							 			} else { if (logger.isInfoEnabled()) logger.info("mismatching receive task"+ nodeElem);}
								 		...
								 		if(collab.Name2PuGtws.containsKey(nodeName))
								 			collab.Name2PuGtws.get(nodeName).add(new Pair<IRole,IGateway>(processId2role.get(processId),gtw));
								 		else {
									 		...
									 		if(collab.Name2PuGtws.containsKey(nodeName))
							 					...
							 					edges.add(nodeElem);
							 				else if (logger.isInfoEnabled()) logger.warn("Unprocessed Element: " + nodeElem.getName() + ", id: " + nodeElem.getAttributeValue("id"));
				}
			}
			...
			//building the public model and filling the role2Public model Map of the collaboration
			if (digraph != null){
                ...
                if (logger.isInfoEnabled()) logger.info("public model added: "+p.getName());				
			}
		...
		//this method returns the mapping between the public nodes and the matching choreography nodes in the choreoModel
		...
		//this method returns the mapping between the choreography nodes and the pair nodes in the matching public modes
		public Map<IChoreographyNode, Pair<IPublicNode,IPublicNode>> getChoreoNode2PuNPairMap(IChoreographyModel cm){
			...
			for(IChoreographyNode cn: cm.getdigraph().getVertices()){
				...
				if(aux==1 && first!=null && second !=null )
					...
					logger.fatal("Matching problem chorNode2PuNodePair: "+cn.getId()+" do not have corresonding public nodes");
			}
		...
		//this method returns the mapping between the choreography gateways and the set of gateways in the matching public modes. it also fills the map between
...
/*	public  Map<Pair<IPublicNode,IPublicNode>,IChoreographyNode> getMapPr2puNode(IChoreographyModel cm){
