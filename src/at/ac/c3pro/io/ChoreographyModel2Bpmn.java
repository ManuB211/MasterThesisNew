...
public class ChoreographyModel2Bpmn {
    ...
    private final List<Element> seqFlows = new ArrayList<Element>();
    private final List<Element> chorTasks = new ArrayList<Element>();
    private final List<Element> gateways = new ArrayList<Element>();
    private final List<Element> events = new ArrayList<Element>();
    private final List<Element> bpmnShapes = new ArrayList<Element>();
    private final List<Element> bpmnEdges = new ArrayList<Element>();
        ...
        for (IChoreographyNode node : chorNodes) {
            ...
            if (node instanceof Interaction) {
                ...
                if (node.getMessage() != null) {
                    ...
                    System.out.println(msgFlowId);
                    choreoTask.addContent(new Element("messageFlowRef", BPMN2NS).setText("sid-" + msgFlowId));

                    Element msgFlow = new Element("messageFlow", BPMN2NS);
                    msgFlow.setAttribute(new Attribute("id", "sid-" + msgFlowId));
