...
public class ChoreographyGenerator {
	...
	public static Choreography generateChoreographyFromModel(IChoreographyModel chorModel) {
		...
		public List<IPrivateModel> privateModels= null; 
		public String id;
		...
		for(Role role : choreography.collaboration.roles){
			IPublicModel pum = ChoreographyGenerator.transformChorModel2PubModel(chorModel.projectionRole(role, true), role);
		...
		for(IPublicModel pumS : choreography.collaboration.puModels)
			for(IPublicNode nodeS : pumS.getdigraph().getVertices()){
				if(nodeS instanceof IGateway){				
					Pair<IRole, IGateway> pairS = new Pair<IRole, IGateway>(choreography.collaboration.PuM2R.get(pumS),(Gateway) nodeS);
					...
					for(IPublicNode pun : pum.getdigraph().getVertices()){
						if( pun instanceof IGateway && chorNode.getName().equalsIgnoreCase(pun.getName())){
							...
							if(choreography.ChGtw2PuGtws.containsKey(chorNode))
								choreography.ChGtw2PuGtws.get(chorNode).add(new Pair<IRole, IGateway>(choreography.collaboration.PuM2R.get(pum),(Gateway) pun));
							else{
								Set<Pair<IRole, IGateway>> set = new HashSet<Pair<IRole, IGateway>>();
								set.add(new Pair<IRole, IGateway>(choreography.collaboration.PuM2R.get(pum),(Gateway) pun));
...
//		}
	...
	public static IPublicModel transformChorModel2PubModel(IRpstModel<Edge<IChoreographyNode>, IChoreographyNode> choreoM, Role currentRole){
		...
		for(IChoreographyNode node : choreoM.getdigraph().getVertices()){
			...
			if(node instanceof Event){
				...
				if(((Interaction) node).getSender().name.equals(currentRole.name))
					C2Pnode.put(node, new Send(((Interaction) node).getReceiver(),((Interaction) node).getMessage(),((Interaction) node).getName()));
				else if(((Interaction) node).getReceiver().name.equals(currentRole.name))
					C2Pnode.put(node, new Receive(((Interaction) node).getSender(),((Interaction) node).getMessage(),((Interaction) node).getName()));
				else  System.out.println("currentrole "+currentRole +"not identified in interaction "+node);
					
			}else {System.out.println("Unknown type of node : " + node);}	
		}
